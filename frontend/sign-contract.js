...
async function loadContract() {
  ...
  const res = await fetch(`${API_BASE}/contracts/sign/view/${encodeURIComponent(token)}`);
  const data = await res.json();
  if (!res.ok) throw new Error(data.detail || "Contract unavailable");
  document.getElementById("contract-meta").textContent =
    `${data.template_id.toUpperCase()} · ${data.contract_number} · ${data.customer_legal_name}`;
...
document.getElementById("sign-form")?.addEventListener("submit", async (event) => {
  ...
  try {
    const res = await fetch(`${API_BASE}/contracts/sign/${encodeURIComponent(token)}`, {
      method: "POST",
